...
try:
    ...
    # Set objective
    m.setObjective(x + y + 2.1 * z, GRB.MAXIMIZE)
...
m.setObjective(x + y + 2.1 * z, gp.GRB.MAXIMIZE)
...
for l in islice(f,1,None):
    ele_load.append(float(l[1]))#我猜是每一列（好像复杂了一点啦啦啦
...
with open("E:\\自动化85\\课程\\大四上\\毕设\\半自动出方案报告\\1.24讨论内容\\升级版code\\main_input.json",encoding = "utf-8") as load_file:
    input_json = json.load(load_file)
print(input_json)

print(input_json['price']['TOU_power'])
print(dict_load['ele_load'])
print(input_json['carbon']['alpha_e'])
print(sum(dict_load['ele_load'])*input_json['carbon']['alpha_e'])
